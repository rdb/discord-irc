...
class Bot {
  constructor(options) {
    ...
    this.ircNickBold = options.ircNickBold === true; // default to false
    ...
    // Keep track of activity of users on IRC channels
    this.channelUserActivity = {};
  ...
  attachListeners() {
    ...
    this.ircClient.on('join', (channelName, nick) => {
      ...
      if (this.ignoredIrcUser(nick)) return;
      if (!this.isIrcUserActive(channel, nick)) return;
    ...
    this.ircClient.on('part', (channelName, nick, reason) => {
      ...
      if (reason) {
        ...
        this.sendExactToDiscord(channel, `**${nick}** has left the channel`);
      }
    });
    ...
    this.ircClient.on('quit', (nick, reason, channels) => {
      ...
      channels.forEach((channelName) => {
        ...
        if (reason) {
          ...
          this.sendExactToDiscord(channel, `**${nick}** has quit`);
        }
      });
    });
  ...
  isIrcUserActive(channel, user) {
    const userMap = this.channelUserActivity[channel.toLowerCase()];
    if (!userMap) {
      return false;
    }
    const time = userMap.get(user);
    if (!time) {
      logger.debug("IRC user " + user + " was never active on " + channel);
      return false;
    }
    const diff = Date.now() - time.getTime();
    logger.debug("IRC user " + user + " was active on " + channel + " " + (diff / 60000) + " minutes ago");
    return diff < 60 * 60 * 1000;
  }

  setIrcUserActive(channel, user) {
    let userMap = this.channelUserActivity[channel.toLowerCase()];
    if (!userMap) {
      userMap = new Map();
      this.channelUserActivity[channel.toLowerCase()] = userMap;
    }
    userMap.set(user, new Date());
  }
  ...
  sendToIRC(message) {
    ...
    if (ircChannel) {
      ...
      if (this.ircNickBold) {
        displayUsername = colors.bold(displayUsername);
      }
  ...
  sendToDiscord(author, channel, text) {
    ...
    this.setIrcUserActive(channel, author);
    ...
    const withMentions = withFormat.replace(/@([^\s#]+)#(\d+)/g, (match, username, discriminator) => {
      ...
      if (reference == 'here' || reference == 'everyone') {
        logger.info('Substituting ' + match + ' mention');
        if (atRef) {
          return '@\u200B' + reference;
        } else {
          return match;
      ...
      // Don't do partial matches for begin-matches.
      if (startRef) {
        return match;
      }
